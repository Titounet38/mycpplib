...
inline BSTR CharPtrToBSTR(const char * charPtr);
inline BSTR WcharPtrToBSTR(const wchar_t * charPtr);

#pragma pack(push, 4)

template <class T>
struct VBA_SAFEARRAY {
	USHORT         cDims;
	USHORT         fFeatures;
	ULONG          cbElements;
	ULONG          cLocks;
...
void aVectToSafeArray(SAFEARRAY *& sa, const aVect<T> & t) {
	...
	auto ptr = GetEl((VBA_SAFEARRAY<double>*)sa, 1);
...
template <class T>
T * GetEl(VBA_SAFEARRAY<T> * ptr, ptrdiff_t i, bool errorIfOutOfRange = true) {
